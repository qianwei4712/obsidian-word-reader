...
- 支持搜索当前工作表、缩放或适配宽度、矩形区域选择，并分别复制显示值或公式。
...
- 使用鼠标拖拽选择矩形区域，按 `Shift` 点击扩展选区，或使用方向键、翻页键、
  `Home` 和 `End` 移动当前单元格。
- 在当前工作表的显示值和公式文本中搜索；按 `Ctrl`/`Cmd` + `F` 聚焦搜索，
  使用 `Enter`/`Shift` + `Enter` 在结果间跳转。
- 默认复制显示值；需要公式时使用独立的“复制公式”操作。插件绝不重算公式。
- 在公式栏查看当前单元格的公式文本和工作簿已有缓存结果。
...
- 工作表解析、切换、搜索和计划中的网格渲染都可取消；包缓存有固定上限，并在
  重新加载、切换文件和关闭视图时释放。
